...
from src.models.train_models import get_regression_models
...
def get_parameters(variable: str) -> Dict:
    ...
    parameters["period"] = [
        TimePeriod(name="200501_201012", start="Jan-2005", end="Dec-2010"),
    ]
...
def experiment_step(
    params: Dict, smoke_test: bool = False, subsample: Optional[int] = 10_000
) -> None:
    # ======================
    # experiment - Data
    # ======================
    ...
    # # standardize data
    y = density_cube_df.iloc[:, 0][:, np.newaxis]
    X = density_cube_df.iloc[:, 1:]

    # ======================
    # experiment - Methods
    # ======================
    res = get_regression_models(X, y, subsample=subsample)
...
def main(args):
    ...
    if args.smoke_test:
        iparam = parameters[0]
        result_df = experiment_step(iparam, args.smoke_test)
    ...
    # initialize datast
        ...
        with tqdm(parameters) as pbar:
            for iparam in pbar:
                ...
                save_path = RES_PATH.joinpath(
                    f"{args.save}_reg_{args.dataset}_{iparam['region'].name}.csv"
                )
